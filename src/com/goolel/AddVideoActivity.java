...
public class AddVideoActivity extends AppCompatActivity {
		...
		uploadVideoButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (titletext.getText().toString().equals("")) {
					new AlertMessage(AddVideoActivity.this).showAToast(R.string.error_title_empty);
					return;
				} else if (titletext.getText().toString().length() > 15) {
					new AlertMessage(AddVideoActivity.this).showAToast(R.string.error_title_limit);
					return;

				} else if (videopathtext.getText().toString().trim().equals("")) {
					new AlertMessage(AddVideoActivity.this).showAToast(R.string.error_path_empty);
					return;
				} else {
					if (sizeInMb > 20) {
						new AlertMessage(AddVideoActivity.this).showAToast("Please select file size less than 20mb.");
					} else {
						checktitle();
					}
				}
	...
	public void generateListView(boolean isCapture) {
		...
		if (isCapture) {
			...
			if (fileUri != null) {
				imagePath = ImagePath.getPath(AddVideoActivity.this, fileUri);
			} else {
				new AlertMessage(AddVideoActivity.this).showAToast("Can't get  path");
				return;
			}
		}
	...
	private class UploadFileToServer extends AsyncTask<String, Integer, String> {
		...
		private String uploadFile(String filePath) {
			...
			try {
				...
				String userId = new DevicePreferences().getString(AddVideoActivity.this, Constants.PREF_USER_ID, "");
	...
	private void checktitle() {
		String extraParameters = new DevicePreferences().getString(AddVideoActivity.this, Constants.PREF_USER_ID, "");
		...
		new RestWebservices(AddVideoActivity.this) {
			public void onSuccess(String data, com.restservice.HttpResponse response) {
				try {
					...
					if (obj.getStatus().equalsIgnoreCase("1")) {
						...
						new AlertMessage(AddVideoActivity.this).showAToast(R.string.error_title_exists);
						titletext.setText(" ");
